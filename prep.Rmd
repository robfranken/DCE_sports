...
<!-- @RF: to-do, match other ego-level covariates for descriptive statistics and moderational analyses (sports history, age, ...) --->
...
clean environment, but keep `fsave` functions

```{r, eval=F}
rm(list=setdiff(ls(), c("fsave")))
```
...
### motivations
I need to match ego-level covariates for moderational analyses. Especially sports motivations (`V10`).
...
mot %>%
  ...
  mutate(across(!id, scale)) -> mot.nm #standardize indicators, expect id.
```
...
```{r, eval=F}
BIC <- mclustBIC(mot.nm %>% select(-id))
ICL <- mclustICL(mot.nm %>% select(-id))
...
```{r, eval=F}
m2 <- Mclust(mot.nm %>% select(-id), modelNames="EEV", G=2, x=BIC)
m3 <- Mclust(mot.nm %>% select(-id), modelNames="EEV", G=3, x=BIC)
m4 <- Mclust(mot.nm %>% select(-id), modelNames="EEV", G=4, x=BIC)
m5 <- Mclust(mot.nm %>% select(-id), modelNames="EEV", G=5, x=BIC)
m6 <- Mclust(mot.nm %>% select(-id), modelNames="EEV", G=6, x=BIC)
m7 <- Mclust(mot.nm %>% select(-id), modelNames="EEV", G=7, x=BIC)
m8 <- Mclust(mot.nm %>% select(-id), modelNames="EEV", G=8, x=BIC)
```
...
#match covariates, here only gender (ref=male)
df$female <- df$GESLACHT2 - 1
#X to NA
df$female <- ifelse(df$female>1, NA, df$female)
...
for (set in 1:3) {
  ...
  for (choice in 1:3) {
        ...
        data$esteem <- data[, names(data) == esteemlist[[set]][choice]]
        data$chosen <- data[, names(data) == chosen[set]]
        data <- data[, names(data) %in% c("id", "female", "set", "options", "prestatie", "kennis", "gezellig",
            "esteem", "chosen")]
        if (set == 1 & choice == 1) {
            df_long <- data
        } else {
