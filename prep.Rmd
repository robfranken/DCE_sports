...
- `Hmisc`: containing `rcorr()` function to create correlation matrix
- `ggcorrplot`: visualize correlation matrix
- `ggplot2`

```{r, eval=FALSE}
packages = c("tidyverse", "mlogit", "haven", "sjlabelled", "Hmisc", "ggcorrplot", "reshape2", "ggplot2")
...
#prop.table(table(df2$activeW2)) #64 percent currently active, at w2
...
for (set in 1:3) {
  ...
  for (choice in 1:3) {
        ...
        data$esteem <- data[, names(data) == esteemlist[[set]][choice]]
        data$chosen <- data[, names(data) == chosen[set]]
        data <- data[, names(data) %in% c("id", "female", "activeW2", "activity.index", "sportsfreq", "set", "options", "prestatie", "kennis", "gezellig",
            "esteem", "chosen")]
        if (set == 1 & choice == 1) {
            df_long <- data
        } else {
...
cor(df$female,df$activeW2, use="complete.obs")
?cor
```
